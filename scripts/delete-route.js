...
function initMap1() {
    ...
    // localStorage.setItem('deletedRoutes', JSON.stringify([]));

    let deletedRoutes = JSON.parse(localStorage.getItem('deletedRoutes')) || [];

    for (var i = 0; i < polylineReferences.length; i++) {
        if (deletedRoutes.busNumber===polylineReferences[i].busNumber) {
            polylineReferences[i].setMap(null);
        }
    }
    ...
    deletedRoutes.forEach((deletedRoute) => {
        const deletedFileName = `coordinates${deletedRoute}.xlsx`;
        const indexToDelete = cD.indexOf(deletedFileName);
        if (indexToDelete !== -1) {
            cD.splice(indexToDelete, 1);
        }
    });

    cD.forEach((file, index) => {
        createPolylineForExcelFile(file, strokeColors[index], index + 1);
    });
    
    localStorage.setItem('AvailableRoutes', JSON.stringify(cD));
...
function createPolylineForExcelFile(file, strokeColor, busNumber) {
    // Load coordinates from Excel sheet
    loadCoordinatesFromExcel(file, function (coordinates) {
        var routeCoordinates = coordinates.map(function (coord) {
            return { lat: coord.lat, lng: coord.lng };
        ...
        // Store the reference to the polyline along with the bus number
        routePolyline.busNumber = busNumber;
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Load deleted routes from localStorage or initialize an empty array
    let deletedRoutes = JSON.parse(localStorage.getItem('deletedRoutes')) || [];

    deleteRouteButton.addEventListener('click', function () {
        // Get the bus number entered by the user and convert it to a number
        var busNumber = parseInt(busNumberInput.value.trim(), 10);

        // Find the index of the polyline reference with the corresponding bus number
        var indexToDelete = -1;
        for (var i = 0; i < polylineReferences.length; i++) {
            if (polylineReferences[i].busNumber === busNumber) {
                indexToDelete = i;
                break;
            }
        }
        ...
        if (indexToDelete !== -1) {
            ...
            polylineReferences[indexToDelete].setMap(null);

            // Remove the reference from the array
            polylineReferences.splice(indexToDelete, 1);

            // Add the deleted route to the deletedRoutes array
            deletedRoutes.push(busNumber);

            // Store the updated deletedRoutes array in localStorage
            localStorage.setItem('deletedRoutes', JSON.stringify(deletedRoutes));

            console.log(`Route for bus ${busNumber} has been permanently deleted.`);
        } else {
            console.log(`Route for bus ${busNumber} not found.`);
        }
    });
});
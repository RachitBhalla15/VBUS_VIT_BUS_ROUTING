...
function initMap1() {
    ...
    let addedRoutes = JSON.parse(localStorage.getItem('addedRoutes')) || [];
    ...
    let cD = ['coordinates1.xlsx', 'coordinates2.xlsx', 'coordinates3.xlsx','coordinates4.xlsx','coordinates5.xlsx'];
    var strokeColors = ['#FF0000', '#000000', '#1E2F97','#F09876','#00BB54']; 

    cD = cD.filter((route) => addedRoutes.includes(route.split('coordinates')[1].split('.xlsx')[0]));
...
function createPolylineForBus(busNumber) {
  ...
  */
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    // Load added routes from localStorage or initialize an empty array
    let addedRoutes = JSON.parse(localStorage.getItem('addedRoutes')) || [];
    ...
    addRouteButton.addEventListener('click', function () {
        ...
        // Check if the bus number is valid (e.g., not NaN)
        if (!isNaN(busNumber)) {
            // Construct the coordinates file name for the new bus route
            var newCoordinatesFile = 'coordinates' + busNumber + '.xlsx';
  
            // Check if the route with the same bus number already exists
            if (addedRoutes.includes(busNumber)) {
                console.log(`Route for bus ${busNumber} already exists.`);
            } else {
                // Add the new coordinates file to the cD list
                cD.push(newCoordinatesFile);
  
                // Create the polyline for the new bus route
              //  createPolylineForExcelFile(newCoordinatesFile, 'your_stroke_color_here', busNumber);
  
                // Add the added route to the addedRoutes array
                addedRoutes.push(busNumber);
  
                // Store the updated addedRoutes array in localStorage
                localStorage.setItem('addedRoutes', JSON.stringify(addedRoutes));
  
                console.log(`Route for bus ${busNumber} has been added.`);
            }
        } else {
